...
Two things these two annotations deliberately do **not** claim, and the honest reading needs both.
...
`task lifecycle` is **not** annotated, and the reason is the stem: "use asupersync regions for **daemon work**". The lifecycle state machine exists — `WorkerState::status_token` emits RFC 0026's six `TaskStatus` spellings so a worker and a `TaskEntry` name the same state, and every illegal transition is one of nineteen typed `RegionFault` values — but no daemon work runs in a region yet. `continuumd`'s `TaskTable` (bn-18z) is still the sole owner of a task's status, and moving it onto the region layer is a designed seam (`TaskEntry` gains a `RegionId`; `task.cancel` calls cancel → drain → finalize and reads its nullable `continuation` off the resulting `CancelOutcome`) rather than a done change. Claiming the bullet on the strength of a seam is the generosity PR 5's bones declined for the same reason.
