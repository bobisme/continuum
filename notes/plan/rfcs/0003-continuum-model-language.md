...
- **Lowering.** A state variable of an instantiated sort lowers to the integers `0..size`; a constant bound to an integer, a Boolean, or a sort element lowers to that literal; equality of sort values is integer equality. Everything else lowers, or is refused, exactly as without a configuration.
