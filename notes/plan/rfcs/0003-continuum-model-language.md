...
**Scope.** Fairness lowers since correction 5 (bn-1ln12, below). Relational variables of composite type are not in this correction and keep `cml.lower.non_integer_state`.

**Example (implemented, bn-23hzh; fairness bn-1ln12).** Under the schema example configuration, which carries `"bounds": {"Nat": {"max": 1}}`, the replicated register has 14 slots — `chosen[0]`, `chosen[1]` (`0..=2`); `stable[a]?` and `stable[a]![0]`, `stable[a]![1]` for each of three nodes; `alive{a}`, `alive{b}`, `alive{c}` — and 50 actions (12 `Stabilize`, 32 `Choose`, 3 `Crash`, 3 `Recover`). Its init domain has 419,904 flat states, within `MAX_INIT_ENUMERATION`, of which one is initial. `"max": 2` gives 34,012,224, which is `cml.lower.init_domain_too_large`. Its reads of `stable[n]` give the predicates `Stabilize#defined`, `Choose#defined`, and `StableWitness#defined`; `chosen.get(e)` is total. `crates/continuum-cml-elab/tests/register.rs` explores it with the reference engine and checks its reachable states, transitions, invariant verdicts, and undefined reads against a hand-written simulation. Its `fairness weak Recover` lowers to one weak assumption over `Recover(n=a)`, `Recover(n=b)`, and `Recover(n=c)` (correction 5).

### Fairness (correction 5)

**Status: in force as design; implemented (bn-1ln12).** `fairness weak X` and `fairness strong X` lower. The typed refusal `cml.lower.fairness` is withdrawn: no model reaches it.

**The model.** `continuum-model-core`'s `Model` carries docs/02 §2's \(F\): a list of fairness assumptions, each a strength (`weak` or `strong`) and a non-empty set of the model's actions, its *scope*. The scope is *enabled* at a state when one of its actions is enabled there, and *taken* by a step when the step's action is in it. Over an infinite labelled execution, weak fairness holds when a scope that is enabled at every state from some point on is taken infinitely often, and strong fairness holds when a scope that is enabled infinitely often is taken infinitely often (RFC 0008, "Fairness scopes"). A model with no assumption has none: there is no default. Strong fairness is in the model because docs/02 §8 (`StrongFair`), RFC 0008, and RFC 0015 name it.

**One assumption per schema, over the union.** `fairness s X` is *one* assumption of strength `s` whose scope is every programmatic action that `X` lowers to: each parameter instance `X(p=u,…)`, each relational candidate `X(…)[r=c,…]` of each instance, and, when `X` is a choice, the same for every action that it offers. It is not one assumption per instance. RFC 0008 and RFC 0015 attach fairness to the action *schema*, and docs/02 §2 gives a schema one relation over all its parameters, \(T_a \subseteq S \times Params_a \times S\). So the schema is enabled when some instance is enabled, and it is taken when some instance is taken. Per-instance fairness is a strictly stronger assumption: under `fairness weak Recover`, the register may keep `b` crashed forever while `a` crashes and recovers, and per-instance fairness excludes that execution. A lowering that chose per-instance fairness would strengthen an assumption, which INV-011 makes a privileged intent revision. This is also the reading of TLA+ `WF_vars(\E n: Recover(n))`, up to the view (next paragraph). `fairness weak A, B` is two assumptions, one for each name, as two lines are. A choice `action N = A | B` with `fairness weak N` is one assumption over the union.

**The view is the label.** A step of an action in the scope is *taken* whether or not it changes the state, and a scope is *enabled* where a guard of one of its actions holds. This is the labelled reading of docs/02 §2 ("a labeled event structure") and of `continuum-kernel-temporal`'s fair-cycle check. TLA+ `WF_v(A)` reads `<<A>>_v`, the steps of `A` that change `v`. The two readings differ only on a step of `A` that leaves `v` unchanged. A port that depends on this difference states its view explicitly (docs/25, "named fairness monitors"). A view other than the label is not implemented, and CML has no syntax for one. The identity encodes no view, so every fairness identity means the label view. A later view needs its own section tag in the encoding, which is an identity change (plan §4.6).

**Names.** A scope names its actions with the functions that name the actions themselves (`instance_label`, `candidate_label`), so a scope lists exactly the actions that `X` lowered to. The names are as injective as the action names are (correction 4, "Action schemas").

**Canonical form and identity.** The builder resolves each name to its action by binary search over the sorted actions. It holds a scope as the ascending set of those indices (a repeated name is one member), and it holds the assumptions sorted by strength and then by scope, with a repeated assumption collapsed. An unknown name is `ModelError::UnknownAction`, an empty scope is `ModelError::EmptyFairness`, and more than `MAX_FAIRNESS = MAX_ACTIONS` declared assumptions is `ModelError::TooMany`. The lowering checks the last limit in its preflight, before any scope is built. The assumptions are part of `Model::identity` (RFC 0015: "Adding a fairness assumption changes the claim identity"): after the predicates, and only when there is at least one assumption, the encoding writes their count and then, for each assumption, a strength byte (`0x20` weak, `0x21` strong), the scope's size, and the scope's action names in ascending order. A model without fairness keeps its `continuum-model/1` identity byte for byte (`crates/continuum-model-core/tests/identity_golden.rs`). The encoding stays injective, because the part before the section is self-delimiting. The run identity changes with the model identity.

**Resources.** Fairness is lowered last, after the invariants. The action and choice tables hold references and are charged before they are built. Each assumption is one planned site. The plan computes each covered action's instances, candidates, and longest name, as the preflight does. The plan predicts per member the output (the name, the builder's resolved copy, and the scope index) and the work (writing and resolving the name, and per instance its values' text). The site checks the work and charges the output before the first name is written, and it spends exactly its prediction. The builder's resolution (one binary search per member) and sorts, and the model identity's walk over the assumptions, are charged before they run.

**Liveness.** The reference engine's `liveness` module checks `◇ P` and `□◇ P` for a declared predicate `P` under the model's own assumptions. It uses an Emerson–Lei fair-cycle search over a closed exploration. A counterexample is a lasso. A bounded exploration is inconclusive, never a pass. The caller states where a behaviour may stutter. The choices are everywhere, only at terminal states, or never. Everywhere is CML's `stutter(state)` and the reading of a lowered model, so a model without fairness guarantees no progress. Only at terminal states is RFC 0015 `stutter-forever`, the maximal-execution reading, which is itself a progress assumption. Never is `deadlock-violation`. There is no default. The replicated register's `□◇ (some node alive)` holds under its fairness and fails without it, and `□◇ (b alive)` fails under it and holds only under the stronger per-instance reading. An independent fair-cycle search over the hand-written simulation agrees on all six verdicts (`tests/register.rs`). The kernel's fair-SCC certificate (`continuum-kernel-temporal`) still carries weak fairness of single actions only. A certificate for a union scope or for strong fairness needs a wire extension, which is not in this correction.

### Pinned init (correction 6)

**Status: implemented (bn-2ri63).** Init enumeration (correction 4) evaluates the init predicate at every canonical state of the whole slot domain, so a model whose domain passes `MAX_INIT_ENUMERATION` is `cml.lower.init_domain_too_large` even when its init fixes every variable to one value. The durable register with a crash counter at claim-A scope is such a model: `3 · 2^22` flat states, one initial state.

**The path.** A lowering takes an init path: `Enumerate`, the whole domain (the default, and what `lower`, `lower_with`, and `lower_configured` do), or `Pinned` (`lower_with_init`, `lower_configured_with_init`). The lowered init predicate is a tree of `&&` over conjuncts. A conjunct *pins* a variable `x` when it is `x == c`, `c == x`, or a `||` tree whose every leaf is such an equality over the same `x`. On the pinned path, each variable's *axis* is the constants of the first conjunct, left to right, that pins it, kept inside the variable's domain, ascending and without repeats. A variable that no conjunct pins keeps its whole domain as its axis. The candidates are the product of the axes, in the order of the whole-domain enumeration (the last variable fastest, each axis ascending). Each candidate is checked as the whole-domain enumeration checks it: the canonicity constraint, then the predicate.

**When it applies.** Only when no evaluation can fail at any state: the predicate and the canonicity constraint contain no arithmetic node (the one evaluation error that depends on the state), every variable read names a declared variable, and every node lies within `MAX_EXPR_DEPTH` as the evaluator counts depth. The predicate must also have no definedness condition (no partial map read `m[k]`; the total `m.get(k)` has none), and the slot names must be distinct, which a hand-built model may break. The whole-domain enumeration refuses an undefined read at *any* canonical state (`cml.lower.undefined_read`), also a state that the predicate rejects, and only the whole domain finds that state. A map read of a scalar value decodes its slot with arithmetic, so it fails the first condition too, and so does a negative literal, which the lowering writes as `0 - n`: conservative, never inexact. Otherwise the pinned path enumerates the whole domain.

**Exactness.** A state that the predicate accepts satisfies every conjunct, because `&&` is true only when both operands are true. So the state lies on every axis. Under the conditions above, the whole-domain enumeration raises no evaluation error and no undefined-read refusal. Both paths hand the builder the same initial states in the same order, so `cml.lower.too_many_initial_states` and the builder's own errors (such as no initial state) fall at the same state, and the lowered model and its identity are the same. The paths differ only in which candidates they charge. So they can differ only in a resource refusal: `cml.lower.init_domain_too_large`, `cml.lower.work_limit_exceeded`, or `cml.lower.output_too_large`.

**Resources.** The analysis is planned before it runs. A position table (one entry per variable, and its sort) is charged before it is built. The work of two scans — a totality scan over the canonicity constraint and the predicate, and a pin scan over the predicate, each with one position lookup per node — plus the sort of at most one value per predicate node, is spent in one step before the first scan. The sizes come from the measures that the lowering already holds. The axes (per variable a pin entry, an axis, a cursor, and a candidate value, and two copies of each pinning leaf that the totality scan counted) are charged before any of them is allocated, and each is allocated at a capacity fixed before it is filled, never grown (cr-1s123d). The scans are iterative; their stacks, not charged, hold at most one pending operand per level of an expression no deeper than `MAX_EXPR_DEPTH`. The enumeration is then charged as the whole-domain enumeration is charged: its candidate count, the product of the axes, is checked against `MAX_INIT_ENUMERATION`, and `init_work` over that count is spent before the first candidate. An unpinned variable keeps its whole domain on its axis, so no init shape enumerates past the bound. A pinned lowering replays under the limits that it reported.

**Evidence.** `crates/continuum-cml-elab/tests/init_pinned.rs` checks both paths against each other over the repository corpus, the configured examples, and 600 generated init predicates. With the pinned path, the durable register with at most two counted atomic crashes lowers under the default budget. The reference engine establishes `Agreement`, `AckedIsDurable`, and `OneValuePerSlot` over its 184,512 reachable states, which are exactly the literal atomic-crash closure of the uncounted model (`tests/golden/pr16_impl02_counted_crash.evidence.txt`). A crash adds no `(log, pending, acks)` state there: each of the 61,504 is reachable with no crash, so the crash bound restricts no state invariant. The crash is exercised by the step checks over every `Crash` step and by the seeded bug `neg-07`, refuted within two crashes.
...
5. **Fairness.** Lowering refused every fairness declaration (`cml.lower.fairness`) because the programmatic model had no fairness. docs/02 §8, RFC 0008, and RFC 0015 name weak and strong fairness of actions, but they do not say what fairness of an action schema means once the schema is expanded per parameter, or what a step must be to count as "taken". Normative: "Fairness (correction 5)" above (bn-1ln12). The model carries weak and strong assumptions over sets of actions. `fairness s X` lowers to one assumption over every action that `X` expands to (the union, never per instance, which would strengthen the assumption). A step is taken by its label. The assumptions are part of the model identity in an optional trailing section, so a model without fairness keeps its identity. Direction: the RFC completes docs/02 §8 and RFC 0008 "Fairness scopes" for schemas; no contradiction.
6. **Pinned init.** Correction 4 enumerates the init predicate over the whole slot domain, so a model whose init fixes every variable is refused as `cml.lower.init_domain_too_large` once its domain passes `MAX_INIT_ENUMERATION`, and the durable register could not count its crashes at claim-A scope. Normative: "Pinned init (correction 6)" above (bn-2ri63). An opt-in init path enumerates only the product of the values that the init predicate's pinning conjuncts admit, and only where no evaluation can fail and the predicate has no definedness condition (no partial map read `m[k]`). It gives the same initial states, errors, and typed refusals as the whole domain, except for resource refusals, and it is charged before it runs and bounded by the same `MAX_INIT_ENUMERATION`. The default path is unchanged. Direction: the RFC refines correction 4's init enumeration; no contradiction.
