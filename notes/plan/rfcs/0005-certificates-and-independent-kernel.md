...
5. no read is undefined at a table state (bn-iu8eh): every definedness predicate of an action chain holds there, and, for the invariant class, every definedness predicate that guards the named predicate holds there;
6. for the invariant class, the named predicate of the model holds at every table state.

Step 5 is RFC 0003 "Definedness" and RFC 0013 "Undefined and partial behavior". A model marks an undefined read with a predicate `X#defined` (and, for a hand-built model, deeper `X#defined#defined`). A state where one is false is the typed outcome "undefined read in `X`", an error that invalidates the model, so no closure over it is valid, whoever produced it. The kernel classifies the chains with its own code, by the rule `continuum_model_core::definedness` states: a chain guards the predicate its base names only when every name of it is a declared predicate, none is an action's name or the schema `X` of an instance `X(…)`, and none contains `(`; every other chain is an action's and is checked under every claim (fail closed). Steps 5 and 6 read every table state in ascending order. An evaluation fault ends the check at once. Otherwise an undefined action read is `undefined-action-read`, then an undefined read in the invariant (or a false invariant that is itself a definedness predicate) is `undefined-invariant-read`, then a false invariant is `invariant-violated`. This is the precedence of the reference engine. Each names the least table state of its kind, where the engine names the shallowest reachable one. The kernel reads every table state, not only the reachable ones, as it does for the invariant, so an undefined read at a carried state the model does not reach still refutes the claim. The chain of a predicate the claim does not name does not bear on it. The classification and the scan are charged in step 1. `continuum-engine-reference/tests/kernel_definedness_differential.rs` compares the kernel's classification with the model core's through `check_certificate`.
...
Wire epoch 1 stays decodable for the two-epoch window of ADR-0018. A wire-epoch-1 claim keeps `certificate-model-correspondence` in its trusted components, so the assurance difference is visible in every verdict and receipt. That component also covers definedness: a wire-epoch-1 certificate carries no model, so the kernel cannot read its definedness predicates. Wire epoch 2 defines no state-type family. The invariant class exists only at wire epoch 2.
