...
**Frontier dependency:** sub-file incremental trust is a **ratified** register row (plan §24.5, lane `research/27`, `quote-id=subfile-incremental-trust-mismatch-rate`; ratified by bn-2tt as a lead-ratified proposal that the G5 review may tighten, never loosen). The threshold and the sampling-rate rule live in research/27, not here, and this RFC does not restate them (correction 15). Module-granularity invalidation is the named fallback and MUST remain a supported configuration.
...
- **Granularity.** Declaration, action, property, and effect granularity are used where provenance is sound; file granularity is not the design (docs/42). Sub-file granularity is a ratified frontier row (plan §24.5, research/27, correction 15) whose kill criterion is "dependency capture cannot be made trustworthy below file/module granularity"; module-granularity invalidation is the named fallback and MUST remain a supported, policy-selectable configuration, and a reuse-edge class stays on it until it meets the research/27 threshold.
...
- **Atomicity.** Disk exhaustion or any other publication failure aborts atomically and returns `PublicationAborted`; the result whose publication aborted is not published and nothing is truncated (plan §4.5, INV-017). Atomicity is per artifact: [RFC 0026](0026-continuumd-native-protocol.md) "Atomicity of publication" states what a client may assume when one operation publishes several artifacts (RFC 0026 correction 48). A partially written result MUST NOT become an index entry under any recovery path.
...
- Until that derivation ships, **"1 in 64, uniformly by query key hash" is the labeled bootstrap default and nothing more.** It is not a ratified rate, and a deployment MUST NOT cite it as evidence of a detection probability. The register row that ratifies a mismatch-rate threshold — sub-file incremental trust, plan §24.5, lane research/27 — is ratified there (correction 15), and its rate rule is the larger of this bootstrap and the research/27 sample-size derivation; this RFC does not restate it.
...
9. **The sampling default is a labeled bootstrap, not a rate.** docs/42 states "default 1 in 64, selected uniformly by query key hash, per RFC 0030" as though this RFC ratified it. Normative: the rate MUST derive from plan §8.6's declared confidence target per reuse class, reviewed at G5; "1 in 64" is the labeled bootstrap default only, and the register row that ratifies a mismatch-rate threshold (plan §24.5, research/27, sub-file incremental trust) was **draft** when this correction was written; correction 15 records its ratification. Direction: RFC governs docs/42; the register row is untouched by this RFC.
...
15. **The sub-file trust row is ratified, not draft.** Earlier revisions of this RFC called plan §24.5's sub-file incremental trust row draft, and said it held no threshold. bn-2tt ratified it in research/27 (`quote-id=subfile-incremental-trust-mismatch-rate`): zero clean mismatches over at least 4,603 sampled sub-file reuses per reuse-edge class, which bounds the rate below 1 in 1,000 at one-sided 99 percent confidence. The sampling rate is the larger of the 1-in-64 bootstrap and 4,603 divided by the smallest class's expected sub-file reuse count. Both are a lead-ratified proposal, and the G5 review may tighten them, never loosen them. Normative: this RFC cites the row and does not restate its numbers; the bootstrap stays a bootstrap. Direction: research/27 governs the threshold; this RFC's frontier-dependency, granularity, and sampling text now point to it.
...
- **F2 — the explain responses are free-form strings.** `reasons` is a string map and `edges` a string list, while the nine dependency reasons and four reuse-edge classes are closed enums. INV-003 forbids prose-only machine interfaces; these should be wire enums with prose as a rendering. Mirrors RFC 0031's F4. — **Partly paid (bn-ah1k8)** by the compatible form, on the precedent of RFC 0026 F22: the fields stay `String`, and two rules declare their domain. `rule query.reuse_reasons` keys `reasons` by the handle spelling of each member of `reused` and `recomputed`, one entry each, and gives each value as a reuse-edge class of the table above or a recompute reason token — one of the nine dependency reasons, `key-change`, `unknown`, or `not-memoized`. `rule query.invalidation_edges` spells each member of `edges` as `<handle>:<reason>` over a member of `invalidated` or `unknown`, and states why an edge is a string over a handle: an index record is a cache entry with no content identity to publish. A daemon may emit no other string in these positions, and a client applies this RFC's fail-closed rule to a token it does not recognize. The retype to wire enums changes a declared field's type and stays owed at the next major.
