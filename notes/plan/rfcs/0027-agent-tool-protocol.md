...
|---|---|
| small orthogonal operations | the registry: 75 operations in 19 namespaces, one act each; `rule task.no_generic_start` forbids the omnibus verb |
...
| no prompt or resource may mutate evidence status | A5 — not an adapter rule: no operation in the 75 sets a status |
