...
- **The protocol is at 3.9 as of bn-18w74, and this document did not need to move:** 3.9 adds one error code, `OutcomeUnknown`, and one rule (RFC 0026 correction 60); no operation, level, annotation, scope, or admission test changes.
- **The protocol was at 3.8 as of bn-3glnv, and this document moved with it: the registry is 83 operations in 20 namespaces, and six of the eight new rows are `@privileged`.** IDL 1.16 puts plan §18.6's signing identities on the wire: `signing.mint`, `signing.rotate`, `signing.revoke`, and `signing.sign_pack` at `revise-intent` and privileged; `signing.registry` and `signing.verify` at `read`; and `intent.export_bundle` and `intent.import_bundle` at `revise-intent` and privileged. Every one of the eight also requires a capability with no scope list at all, because a signer is not an instance of any class a scope can name (correction 34). What moved: the table, the distribution (`read` 20, `revise-intent` 10, `@readonly` 30, `@mutation` 53, `@privileged` 11, `@audit_recorded` 13), T3's privileged list, and R-2's scope sentence. What did not: the ladder is the same five levels, the admission predicate the same four tests, `CapabilityDescriptor` the same ten fields, and no existing operation changed level, annotation, or shape.
- **The protocol was at 3.7 as of bn-28kv4, and this document moved with it: `CapabilityDescriptor` has ten fields, and T2's instance scope reaches every handle class but `cap`.** IDL 1.15 adds `instances: list<ArtifactHandle> optional` and `rule capability.instance_scope`, which pays F2 (correction 33, RFC 0026 correction 52). What moved: T2, D4, H1, the descriptor table, and F2's disposition. What did not: the registry is the same 75 operations at the same levels, the ladder the same five levels, and the admission predicate the same four tests. A capability without `instances` is admitted exactly as at 3.6.
- **The protocol was at 3.6 as of bn-3of5h, and this document moved with it: `workspace.create_by_reference` is the 75th operation, and it is the first registry growth that enters an *existing* namespace.** This is the third time the registry has grown, and the reason is neither of the first two: not a defect paid with a verb (correction 30 still forbids that), and not a subsystem arriving on the wire, but a **measured asymmetry between two projections of one operation**. `SnapshotComponents` is the largest argument this protocol carries — 17,208 B over the DX-10 instrument's 24 `workspace.create` calls — and a local CLI never pays it, because it names a corpus port and resolves the components from the filesystem, while "a remote protocol client must transmit `SnapshotComponents` in full" (PR-10 IMPL-02, quoted in `notes/START_HERE_IMPLEMENTATION.md`). `rule conformance.adapter_parity` requires adapters to yield identical artifacts, and they do; what differed was what each had to *say* to get them. This verb closes that, and `notes/DX10_BYTE_LEDGER.md` §8 is why it is the only one of seven redesign candidates that could be closed inside major 3 — the other six move a presence marker on a `required` member or rewrite a rule, and `rule versioning.compatible_change` is a closed list of five. **The level is `propose`, and the family's own row decides it**: "Creating, forking, and sealing publish candidate snapshots", and this publishes one — a `ws_` a caller may then fork, seal, and verify against. The reading that would put it at `read` — naming content the daemon already holds is an inspection — is the argument `context` is `read` on, and it does not transfer: `context.compile`'s `ctx_*` is "the daemon's own record of a read", whereas a `ws_` is the artifact `workspace.create` publishes, reachable by the same caller under another name. A `read`-level create would restore snapshot creation to exactly the Reviewer profile this document's own worked example denies `workspace.create` to, which is the widening A7, T4, and INV-015 forbid; it would also be a level split inside one act, which A2 ("every operation carries exactly one minimum level") reads as a property of operations and R-1 refuses to read as a property of namespaces. What moved: the table is 75 rows, `propose` is 10, `@mutation` is 47. What did not: the ladder is the same five levels, the admission predicate the same four tests, `CapabilityDescriptor` the same nine fields, the namespace count is still 19, `@audit_recorded` is still 7, `@task_starting` still 26, and no existing operation changed level, annotation, or shape.
- **The protocol was at 3.5 as of bn-1as8e, and this document moved with it: `whiteboard.compile` is the 74th operation and `whiteboard` the 19th namespace.** This is the second time the registry has grown, and the reason is the same shape as the first: not a defect paid with a verb — correction 30 still forbids that — but a subsystem the dossier had already specified arriving on the wire. plan §11.5's whiteboard compiler, `schemas/whiteboard-note.schema.json`, and RFC 0038's W1–W9 all landed before it with the wire explicitly deferred, and the deferral paragraph named the operation it was deferring. The declared surface was searched before the verb was minted, because serving already-declared vocabulary needs no registry edit at all: nothing in the IDL serves it, and the two neighbours that read as candidates are not (`forge.create`'s `sketch` is RFC 0033's synthesis input and answers with a `ForgeHandle`; `context.compile` runs graph-to-pack, the other way). **The level is `propose`, and the ladder's own definition of level 2 decides it**: "publishing candidate artifacts […] none of which asserts anything about behavior". Every node a note proposes lands at `proposed`, the note format has no status member (RFC 0038 W5), and no compiled node asserts anything at all — so this is the first `evidence`-graph-adjacent verb below `execute`, and that is the point rather than an accident. What moved: the table is 74 rows, `propose` is 9, `@mutation` is 46. What did not: the ladder is the same five levels, the admission predicate the same four tests, `CapabilityDescriptor` the same nine fields, `@audit_recorded` is still 7, `@task_starting` still 26, and no existing operation changed level, annotation, or shape.
- **The protocol was at 3.4 as of the bn-3jrtz bundle (F19/F20), and this document did *not* move with it — no operation, authority level, or annotation changed, so every table below stands.** The registry last moved at 3.3 (bn-3sypm), and that revision is worth keeping in full because it is the one time the registry grew: RFC 0038 decided its F14 trio and `evidence.link` is that decision on the wire: the 73rd operation, the only one that appends an evidence-graph edge, and therefore the first artifact INV-004's `CHECKED_BY` requirement has ever had (plan §2 SD-11). Adding an operation is the first change `rule versioning.compatible_change` names, so 3.2 → 3.3 is a minor and the major window is untouched. The row arrives here, in plan §10.2, and in the IDL together — the F13 precedent, and the third bullet above states it as an obligation rather than a habit. What moved: the table is 73 rows, `execute` is 41, `@mutation` is 45, `@audit_recorded` is 7. What did not: the ladder is the same five levels, the admission predicate the same four tests, `CapabilityDescriptor` the same nine fields, and no existing operation changed level, annotation, or shape. Correction 30 below ("no flag was paid with a new operation") is not overturned: it was a statement about a *sweep*, and this is a decision RFC 0038 owed, taken by the RFC that owed it.
- **The protocol was at 3.2 as of the bn-i4aem revision, and nothing in this document moved with it.** IDL 1.3 / protocol 3.2 reconciles twelve wire defects and fixes the codec's three open decisions (RFC 0026 corrections 42–47). The ladder is the same five levels, the registry the same 72 rows, the admission predicate the same four tests, and `CapabilityDescriptor` the same nine fields. One clause of this document reads differently because the IDL changed under it and not because its meaning did: **R-5** requires an operation registered ahead of its subsystem to fail with `UnsupportedSemanticFeature`, and until 3.2 twenty-five of the seventy-two operations could not return that code under `rule errors.common`. It is now admissible for every operation, so R-5 and the common union no longer disagree — see RFC 0026 correction 42, which owns the change.
- **The protocol was at 3.1 as of the previous revision.** The single minor bump 3.0 → 3.1 (IDL 1.2) covers IDL 1.1's compatible fixes and this document's F1–F8 disposition together: `CapabilityDescriptor.profile`, `CapabilityProfile`, `DataGrant`, `ServerReject`, `ResultEnvelope.audit`, and four new `rule` blocks. Every one is compatible under `rule versioning.compatible_change`. Nothing in the ladder, the registry, or the admission predicate's *structure* moved: the levels are the same five, the rows the same 72, and the tests the same four — T3 gained a wire representation, not a new meaning.
...
T2 (scope)      every snapshot, intent, and artifact class the request names
                ...
                and every other instance it names lies within .instances,
                class by class (3.7)
...
- **T2 is instance-scoped for snapshots and intents, and from 3.7 for every other class but `cap`; it is class-scoped for everything a capability does not list by instance.** An empty list means unrestricted *within the level*, so a non-empty child list is always a narrowing. `instances` is decided class by class: a class none of whose instances is listed keeps its class scope, and a class with a listed instance admits only listed instances (`rule capability.instance_scope`). A request "names" exactly the handles its envelope and arguments carry, so an instance scope does not reach an artifact *derived* from a listed instance (correction 33). A request naming an artifact outside scope is denied, and the denial MUST NOT reveal whether the artifact exists.
- **T3 is what makes "proposal only" and "review only" expressible.** The eleven `@privileged` operations — `intent.accept`, `intent.reject`, `intent.lock`, `intent.export_bundle`, `intent.import_bundle`, `repair.promote`, `repair.reject`, and from 3.8 `signing.mint`, `signing.rotate`, `signing.revoke`, `signing.sign_pack` — are never admitted by level alone; RFC 0026's "never present in a default agent capability profile" is this test. `intent.propose_revision` is *not* `@privileged`, so a capability at `revise-intent` without privileged membership can propose a revision and cannot accept one — which is precisely docs/49's "revise intent: proposal only" cell, and it is unreachable if privilege is read as a level.
...
`CapabilityDescriptor` is what a capability confers. It has eight fields at protocol 3.0, nine at 3.1, and ten at 3.7:
...
| `artifact_classes` | `list<String>` | required | plan §4.4 class tokens, the prefix without its `_` (`rule artifact_class.spelling`); empty means all. A non-token is refused at provisioning |
...
| `instances` | `list<ArtifactHandle>` | optional | 3.7; instance scope for every class except `ws`, `in`, and `cap`, each member's class being its handle prefix. A class with no listed instance keeps its class scope; absent is the 3.6 class scope. A `ws_`, `in_`, or `cap_` member, a member of no class, or one whose class a non-empty `artifact_classes` does not hold is refused at provisioning (`rule capability.instance_scope`) |
...
**Administration is out-of-band at protocol major 3** (RFC 0026, correction 20). Minting, scoping, delegating, and revoking are not operations in the 75, and the reason is A7: a mint reachable from an agent connection is one operation away from a wider grant. The obligations below hold wherever that surface lives. Its landed form is `ReferenceStore::mint` and `ReferenceStore::revoke` under `Action::Administer`, which audits every decision and refuses to mint above the minting capability's own level; `Action::minimum_authority` places `Administer` and `Audit` at `promote`, which is a **store-side floor for an out-of-band action, not a sixth ladder level and not a registry entry**.
...
- **D4** — a child's `snapshots`, `intents`, and `artifact_classes` MUST be subsets of its parent's. An empty parent scope means unrestricted, so a non-empty child scope is always a narrowing. From 3.7 the same holds for `instances` class by class: for every class the parent lists instances of, the child MUST list instances of that class too, and only ones the parent lists. An empty list is the widest scope throughout, so an empty child list narrows only an empty parent list (cr-3hcpn4: before 3.7 landed, an empty child list passed under any parent).
...
- **R1** — revocation takes effect on in-flight connections, at the next request (E1's rule). An open subscription re-runs the whole admission of its request (T1–T4) before every delivery, so a revoked or expired capability, a revoked ancestor, or an ancestor re-provisioned narrower closes it before the next delta frame, with nothing on the wire. The handshake and every request decide the whole stored chain of the presented capability up to its root, also when it is the connection's own, so a delegate that opened its own connection is refused once an ancestor is revoked, narrowed, or expired (D8).
...
| `intent.export_bundle` | revise-intent | `@mutation` `@privileged` `@audit_recorded` |
| `intent.import_bundle` | revise-intent | `@mutation` `@privileged` `@audit_recorded` |
...
| `evidence.link` | execute | `@mutation` `@audit_recorded` |
| `whiteboard.compile` | propose | `@mutation` |
| `signing.mint` | revise-intent | `@mutation` `@privileged` `@audit_recorded` |
| `signing.rotate` | revise-intent | `@mutation` `@privileged` `@audit_recorded` |
| `signing.revoke` | revise-intent | `@mutation` `@privileged` `@audit_recorded` |
| `signing.registry` | read | `@readonly` |
| `signing.verify` | read | `@readonly` |
| `signing.sign_pack` | revise-intent | `@mutation` `@privileged` `@audit_recorded` |
...
|---|---:|---|---:|
| `read` | 20 | `@readonly` | 30 |
| `propose` | 10 | `@mutation` | 53 |
| `execute` | 41 | `@task_starting` | 26 |
| `revise-intent` | 10 | `@paginated` | 6 |
| `promote` | 2 | `@streaming` | 2 |
| **total** | **83** | `@privileged` | 11 |
| | | `@audit_recorded` | 13 |
...
- **R-2** — a namespace prefix is not a capability scope. Scope is expressed in artifact classes and instances (T2), which cut across namespaces: a `dbg_` scope bears on eleven `debug` operations and on `context.compile`'s debugger-branch field alike. The one namespace-shaped rule is the converse and does not break this one: the `signing` operations and the two bundle operations act on the deployment as a whole, so they require a grant with no scope at all rather than a scope of their own (correction 34).
...
- **`workspace.create_by_reference` is `propose`, is `@mutation`, and is deliberately neither `@task_starting` nor `@audit_recorded`.** The level is the family's act: it publishes a candidate snapshot, the same `ws_` `workspace.create` publishes over the same components, so putting it anywhere else would make one act carry two minimum levels and A2 says an operation carries exactly one. The reading worth refuting rather than ignoring is that this is a **read**: it names content the daemon already holds, and inspecting held content is what `read` is for. Three things defeat it. First, what leaves the operation is a *new artifact* the caller may then fork, seal, and start a campaign against — not "the daemon's own record of a read", which is the whole of `context`'s argument for `read` and does not survive being repeated here. Second, the components are held; the *snapshot* is not, and a lineage is opened by this call. Third, and decisively, a `read`-level create would restore snapshot creation to the Reviewer profile whose worked example above denies `workspace.create` by name — a caller narrowed out of one door would walk through a second one that publishes the same artifact, which is the escalation A7 forbids and INV-015 is written against. It is `@mutation` because it opens a lineage and publishes, so its retries must be safe under `rule idempotency.replay`; the request accordingly carries `idempotency_key`, as `workspace.create`'s does. It is **not** `@task_starting`: resolving a commitment and deriving a Merkle identity is a structural transcription over content the daemon already holds, with no engine, no search, and no budget to bound — the shape `evidence.link` and `whiteboard.compile` each follow. And it is **not** `@audit_recorded`, which is a decision and not an omission: R-4's audit set is the acts whose authority cannot be reconstructed from the artifact afterwards, and a `propose`-level snapshot publication is none of them — `workspace.create`, `workspace.fork`, `workspace.seal`, `repair.begin`, and `correspondence.bind` carry none either, and this row matches them. No `data_grant`: a component set the caller can name is one the caller can derive, and the commitment's preimage is the components themselves.
...
- **`evidence` is `read` except for `evidence.link`, which is `execute`.** Verification re-checks a receipt against artifacts; it establishes nothing new and grants nothing, so `evidence.verify` is a read despite being `@mutation`, and `evidence.subscribe` streams *committed* graph deltas and is a read too. `evidence.link` is the one producer-side verb in the namespace: it publishes two new artifacts — a `receipt` node and the `CHECKED_BY` edge naming it — which is the same act `observe.ingest` performs at the same level, and it is emphatically not "the daemon's own record of a read" the way `context.compile`'s publication is. Its extra gate is not a level at all: the checker is the admitted capability's actor and MUST be a `service:` actor, so no level of `agent:` capability reaches it (RFC 0038 "Authority", D3). **No `evidence` operation writes a status**, which is still true and is the point of the split: status promotion is service-restricted (RFC 0038), has no wire verb, and an edge recording that a check ran is deliberately not the promotion that check might license.
- **`signing` spans `read` and `revise-intent`, and its four writes are `@privileged`.** Minting, rotating, and revoking a signer decide which signatures a deployment trusts, and signing a domain pack speaks for the deployment. Each is as consequential as accepting an intent, and a signature over an intent bundle is what the CI acceptance check honors, so each sits beside `intent.accept`: `revise-intent`, `@privileged`, `@audit_recorded`. Reading the registry and verifying a signature assert nothing, so they are `read`. The two bundle verbs join `intent`: exporting signs as the deployment and carries its registry, and importing can enter contracts and adopt revocation records, so both are privileged too, and import still never raises a contract above `proposed` (RFC 0037 I2). All eight require an unscoped grant (correction 34).
- **`whiteboard.compile` is `propose`, is `@mutation`, and is deliberately neither `@task_starting` nor `@audit_recorded`.** The level is level 2's own sentence: it publishes candidate artifacts none of which asserts anything about behavior. A compiled node is a *proposal* — the note format has no status member (RFC 0038 W5), the operation mints no promotion, and every node it appends lands at `proposed` — so it is the counterpart of `repair.begin` and `correspondence.bind` rather than of `observe.ingest`, which is `execute` because a production trace is a capture of real behavior and additionally needs the plan §18.2 grant. It is `@mutation` because it appends to the graph and its retries must be safe (RFC 0038's write model rests on idempotency keys). It is **not** `@task_starting`: compiling a note is a structural transcription over a graph the daemon already holds, with no search, no engine, and no budget to bound — `evidence.link` is the shape it follows. And it is **not** `@audit_recorded`, which is a decision and not an omission: R-4's audit set is exactly the acts whose authority cannot be reconstructed from the artifact afterwards — the five privileged decisions, the grant-gated ingest, and the check attribution INV-004 rests on. A `propose`-level publication of bottom-status nodes is none of those, and the graph is already the append-only record: every compiled node carries `provenance.actor` and names the note in `provenance.inputs`. The other three `propose` publications — `workspace.create`, `repair.begin`, `correspondence.bind` — carry no `@audit_recorded` either, and that is the row this one matches. No `data_grant` either: a note is the caller's own authored document, not someone else's production data.
...
|---|---|
| small orthogonal operations | the registry: 83 operations in 20 namespaces, one act each; `rule task.no_generic_start` forbids the omnibus verb |
...
| no prompt or resource may mutate evidence status | A5 — not an adapter rule: no operation in the 83 sets a status |
...
- **H1 — sharing and isolation are expressed as capability scope.** Shared state is the `snapshots` and `intents` instance scope plus the `ev_` artifact class; isolated state is the `dbg_`, `ps_`, and `forge_` classes, narrowed per sub-agent. From 3.7, per-instance isolation between two sub-agents that hold the same class is expressible as capability scope: each sub-agent's `instances` lists its own `dbg_`, `ps_`, or `forge_` instances while `ev_` stays class-wide, which is why `instances` is decided per class (F2, correction 33). A capability that lists no instance of an isolable class still falls back to the interim rule: the daemon enforces isolation from the creating capability's actor.
...
25. **plan §10.4's evidence-status rule is not adapter-specific.** "No MCP prompt or resource may mutate evidence status" is true of every adapter and of the native protocol, because no operation in the 75 sets a status — `evidence.link` appends an edge and writes no status and `whiteboard.compile` proposes nodes at `proposed`, which is the distinction RFC 0038 "Authority" draws. Normative: A5. Direction: RFC generalizes plan §10.4; no contradiction.
...
30. **No flag was paid with a new operation.** Normative: the registry is 72 rows, the per-level counts are 18/8/40/4/2, and the annotation counts are unchanged by the sweep. This is stated as a decision rather than an observation because three of the five payments had a tempting registry-shaped alternative — an administrative verb for the profile, a handshake operation for the rejection, an audit-read verb for the correlation identity — and each would have breached A7 or RFC 0026's correction 20. A protocol sweep that grows the registry is a redesign, not a sweep. — **Still the rule; not what happened at 3.3.** The registry grew to 73 rows there, and the distinction this correction draws is exactly why that was admissible: `evidence.link` is not a defect paid with a verb, it is RFC 0038 deciding a question it owed (F14) and INV-004's `CHECKED_BY` requirement gaining the artifact it never had. The per-level counts are now 18/8/41/4/2.

### Where this RFC decides the meaning of its own metric

31. **"Interface bytes per solved task" counts the bytes crossing the *agent-visible* interface; a byte no agent reads or writes is not an interface byte.** The ablation bullet above fixes the metric and its denominator and never says *whose* interface it measures, and the PR-10 falsification campaign left exactly that question as its one undecided attack — the label it uses for it is local to `crates/continuum-benchmark`'s suite and is not this document's cross-cutting X1. Two readings were live and they differ in sign rather than in size, so the question could not be left open. Under the agent-visible reading the typed arm is charged its request and result frames while the text arm is charged its command lines and the projection it reads; under the symmetric reading the text arm is additionally charged the CLI process's own daemon frames and a connection handshake per invocation. It is decided on the metric bullet's own stated reason: bytes are the graded denominator *because token counts are model-relative* (B4), and that rationale has purchase only over content some model would otherwise tokenize, which is content entering an agent's context. CLI-internal frames are tokenized by nothing, so no quantity of them can make a token count model-relative, and the symmetric reading therefore finds no support in the reason the text gives for the very rule it purports to read. research/25's derivation of the byte floor corroborates rather than resists: the floor is set to exceed *prompt and format variation*, which is a property of agent context and of nothing else. The rejected alternative is recorded with its argument at full strength: a text proxy charged only for its rendered output externalizes the protocol cost its own process pays, so any surface can be made to look cheap by moving work below the line the metric draws. That argument is real and it is extra-textual — a reason to have written a different metric, not a reading of the one that is written — and adopting it once the numbers were known is the move the campaign that raised the question warned against. Normative: an interface byte is a byte the agent wrote or read at its own interface — the request frame plus the result frame on the typed arm, the command line plus the rendered projection on the text arm — and process-internal traffic is uncounted on both arms alike, by rule and symmetrically rather than as a handicap on either. Correction 17's denominator ruling stands unchanged and is here given its scope; the ratified `quote-id=aci-benchmark-margins` numbers are neither restated nor relaxed. Direction: this RFC decides, supplying the reading its own metric sentence needs and the ratified sentence presupposes. — **Consequence, recorded here because this RFC's own freeze bullet turns on it.** Under the pinned reading the measured margin is −163% against the ratified +30% floor, and a maximal encoding redesign still leaves −86%, so the ratified sentence fires: G0-DX-10 fails as measured and protocol redesign before freeze is in force. That is this document's own rule — if the typed surface loses, the ACI is redesigned, not excused — and the protocol MUST NOT freeze on the surface that was measured. Adjudicated 2026-08-09 under bn-762i, which commissions the redesign; the G0 matrix's DX-10 row carries the measurement, the two measured ties, and the successor work. — **Addendum: the margin this redesign was forced by is unreachable by any lossless protocol, and the measurement that shows it carries four errata and one open question.** The byte-ledger decomposition bn-762i commissioned took the shortfall to field grain and found it structural rather than incidental. With `H` the handshake charged per solved task, `n` the calls per task, `rn` the typed arm's bytes per call and `rs` the text arm's, a passing row is the condition `H + n·rn <= 0.7·n·rs`; on the measured values `H` = 875 B, `n` = 7 and `rs` = 575 B per call, that requires `rn` <= 277 B per call, request and result together. The typed arm measures 1,551 B per call today and the demonstrated post-redesign floor is 679 B per call, so the candidate set travels 68% of the distance and stops on the request envelope, the response skeleton and the payload, which is what the protocol *is*. Stated without the algebra: the text arm's 575 B per call is a lossy rendering of an answer and the typed arm's is a lossless self-describing encoding of the same answer plus the request that asked for it, so the margin asks a lossless encoding of the answer plus the request to come in under half a lossy rendering of the answer alone. The +30% bytes margin is therefore **unreachable under the pinned agent-visible accounting by any lossless protocol**, and a redesign MUST NOT be commissioned with a passing bytes row as its stated goal, because nothing measured can deliver one and a redesign that did reach the row would have become a rendering with extra steps, having thrown away the omission manifests, epoch pinning, assurance envelope and artifact identity that are the typed surface's reason to exist. The exit consequently rests on its second disjunct exactly as adjudicated above — the protocol is redesigned before freeze, and the redesign's obligation is to remove demonstrated waste rather than to flip the row — which the seven successor bones bn-3bn62 raised carry as one measured set: 47.6% of the typed arm's total wire spend, no protocol guarantee dropped, and no extra round trip on any solved task of the instrument's mix. **Four errata to the cost ledger that adjudication relied on, each measured by the same decomposition.** The `epochs` credit is understated by 3.4x, because 8,084 B is the reduction to protocol-only while full removal is 27,864 B and `ServerWelcome.epochs` already delivers the identical set at the handshake. The `SnapshotComponents` credit of 17,208 B is an upper bound rather than an available saving, because a fresh connection creating a fresh snapshot has no other channel by which to establish the reference the credit assumes. `verification.result`, the one operation whose answer carries the semantic verdict, names no artifact on any of its answers. `Omission.recoverable_by` is populated on 0 of 324 omission records, so INV-007's retrieval half is declared on this wire and never once exercised. **One question is recorded open here and is deliberately not decided.** `canonical_cbor` is already a member of the closed `Encoding` enum, already listed in the daemon's encoding registry, already implemented, and simply never offered by the instrument's hello; negotiating it measures at 83% of canonical JSON over the envelope minus its payload, about 816 B per solved task, behavior-only and with no version bump. Whether that counts as reducing what the agent learns is a question about this metric's meaning rather than about the wire, because it shrinks the frames the pinned accounting charges without changing one bit of what the agent reads. It is routed to whoever owns the metric's meaning, recorded as open, and deliberately not commissioned: folding it into a redesign bone would be adopting a byte reduction once the numbers were known, which is the move this correction has already refused once. Measured in [`notes/DX10_BYTE_LEDGER.md`](../notes/DX10_BYTE_LEDGER.md) under bn-3bn62 — §2 for the errata, §4 and §5 for the arithmetic — where every figure is read by re-encoding the landed instrument's own recorded envelopes rather than modelled. — **Erratum to the set figure, which the ledger's own item C7 overstated.** bn-6fuu5 falsified C7 by measurement — the `task.status` summary it projected returns an answer without six `required` and two `nullable` members of `TaskRecord`, which `rule versioning.breaking_change` makes a MAJOR rather than the behavior-only conformance fix the ledger classed it, and the conforming enforcement that did land measures −49 to −255 B per solved task against the projected +1,114 — so the measured set is the six bundled-3.6 items at 4,060 B per solved task and **37.4% of the typed arm's total wire spend** rather than 47.6%, still with no protocol guarantee dropped and now with no extra round trip for any client rather than none on this mix alone, and the unreachability conclusion above is unchanged and strengthened, because the corrected set closes 51% of the gap instead of 65% and the demonstrated post-redesign floor sits above the 679 B per call quoted here — recorded in [`notes/DX10_BYTE_LEDGER.md`](../notes/DX10_BYTE_LEDGER.md) §3 C7 for the falsification and §4 for the corrected sum-check.

### Where the IDL corrected this RFC (bn-3ncfp)

32. **A class scope is spelled in class tokens.** The descriptor table read "plan §4.4 class prefixes", and plan §4.4 writes a prefix with its `_`, so a deployment that followed this table scoped a capability to `ws_` and matched nothing (bn-1slz1, G2-02 finding F3). Normative: `artifact_classes` members are class tokens (`ws`, `dbg`, `ev`), per the IDL's `rule artifact_class.spelling`, and a non-token is refused where the capability is provisioned rather than served as an empty scope. Prose in this RFC that names a class by its prefix (`dbg_`, `ev_`) names the class, not the wire string. Direction: the IDL decides (RFC 0026 correction 49); this RFC is corrected.

### Where this RFC decides at protocol 3.7 (bn-28kv4)

33. **An instance scope is per class, and it does not reach derived artifacts.** F2 left two questions open: the shape of per-instance scope, and whether a scope on one artifact extends to what that artifact produces. The IDL now answers the shape (`CapabilityDescriptor.instances`, RFC 0026 correction 52). Two readings of that list were live. Under the *global* reading, a present list restricts every class to the listed instances. Under the *per-class* reading, only a class with a listed instance is restricted. This RFC takes the per-class reading because H1 needs it. H1's isolation case is a sub-agent that holds its own `dbg_` branch while it shares the `ev_` class with its siblings. The global reading cannot express that, since listing the branch would take away the shared class. On derivation, this RFC was silent: T2 quantifies over what "the request names" and says nothing about a `task_`'s `cont_`, a pack's child `ctx_`, or the `ev_` a campaign produced. The least-authority reading is chosen, and it is also the only decidable one. Admission may not read the store (X3), and deciding that `cont_7` belongs to `task_3` is a store read. So a `task_` instance scope admits no `cont_` by derivation. A derived artifact is in scope only through its own class: listed by instance, or reached by a class scope. A provisioner that wants "this task and its continuation" lists both. A provisioner that wants "this task and no continuation" withholds the `cont` class. The same holds for a write: `task.cancel` and `task.update_budget` on a parked task publish the next revision of its `cont_` without naming it, so the handler decides that instance after admission, where the lookup is allowed, and refuses with the same `CapabilityDenied` before anything is published. A handle carried as a class-agnostic `ArtifactHandle` is decided by the class its own prefix names, so `context.compile`'s root is claimed as a `task` when it is a `task_`, and a handle of no class is denied under any scope (cr-3hcpn4). Derivation is also bounded in the other direction: a handle an operation reaches *from* a named one — a continuation's task, a task's snapshot, intent, and continuation, a pack's snapshot, intent, and evidence, an intent's successor — is decided against the grant before it is used or reported, and a handle the operation creates, converges on, or reports is decided before it is written or returned, held or not: no handle is exempt because it was minted. A continuation a run parks under is decided after the run and before its first write, and a refused run leaves the shared task unchanged, is charged to the presenting capability, and is not run again for it. A replayed outcome is reauthorized under the presenting capability. `rule capability.instance_scope` lists every operation this reaches. Normative: `rule capability.instance_scope`. Direction: this RFC decides where it was silent, and the IDL rule states the decision.

### Where this RFC decides at protocol 3.8 (bn-3glnv)

34. **A deployment-wide operation needs a grant with no scope.** T2 decides what a request names against the grant's lists, and a signer names nothing a list can hold: it is a public key, not an instance of a plan §4.4 class, and the signing operations read or change state that belongs to no part of the deployment — the registry names every signer and the actor of every audit record, and a signature speaks for the whole deployment. Two readings were live. Under the permissive reading, an operation that names no instance passes T2 under any scope, so a capability scoped to one snapshot could mint the deployment's key. Under the fail-closed reading, the operation is outside every proper scope, so only a grant with no scope list reaches it. This RFC takes the fail-closed reading. The six `signing` operations, `intent.export_bundle`, and `intent.import_bundle` are admitted only when the presenting grant's `snapshots`, `intents`, `artifact_classes`, and `instances` are all empty, decided at admission with the rest of T2 and indistinguishable from any other denial (X1). The delegation chain already narrows (D4), so an unscoped grant stands only under unscoped parents. The rule adds no field and no level. The handles these operations create are still decided before they are written — a minted `inb_`, an imported `in_` and its predecessor — so correction 33's derived-handle clause holds unchanged. Normative: `rule signing.identities`. Direction: this RFC decides where T2 was silent; RFC 0026 correction 53 records the wire change.
...
here added an operation, so the then-72-row
registry table above, its five per-level counts, and its seven annotation
counts are untouched by the sweep. (The registry has since grown to 73:
`evidence.link` at protocol 3.3, which is not a flag payment but RFC
0038's own F14 decision — see the versioning bullet above.)
...
- ~~Whether per-instance scope (F2) is expressed as a scope list or as artifact-side ownership, and which of the debugger, the proof service, and Forge can each enforce.~~ **Closed at 3.7 (bn-28kv4):** a scope list, `CapabilityDescriptor.instances`, enforced at admission from the request alone, so none of the three subsystems has to enforce it (F2, correction 33). Artifact-side ownership stays the interim rule for a capability that lists no instance of an isolable class (H1).
