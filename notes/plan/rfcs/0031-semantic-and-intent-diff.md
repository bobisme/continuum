...
Claims are keyed by `id`. A claim present on one side only classifies `added` or `removed`. A claim present on both sides is classified by comparing `expression` under CPNF-1: equal N8 encodings classify `unchanged` (RFC 0037 S1); otherwise the direction is `strengthened` iff behaviors(after) ⊆ behaviors(before), `weakened` iff behaviors(before) ⊆ behaviors(after), `incomparable` iff both inclusions are refuted, and `unknown` otherwise. Both `iff` conditions can hold together for one pair — a proven equivalence under unequal canonical encodings, which CPNF-1's soundness-without-completeness (S1/S2) does not rule out — and a record carries exactly one relation, so neither `iff` may be preferred; R2 also forbids claiming `unchanged` on anything short of encoding equality, and the closed relation set has no `equivalent` token. Such a pair classifies `unknown` (correction 17). A change to `kind` or to the bound `observer` with an unchanged expression is a change of what the claim means and MUST classify `incomparable`, never `unchanged`. A change to `kind` or `observer` together with a moved expression in the same revision classifies `unknown`, not `incomparable`: the content axis's decision procedure answers a question about one claim's meaning under one fixed `kind`/`observer`, and once either has also moved the two sides no longer share the meaning a direction would be decided over, so the content axis's obligation is never discharged across that boundary (correction 19). Renaming an `id` while preserving the expression is `removed` plus `added`, not `unchanged`; the classifier MUST NOT match claims by expression to defeat the rename.

### `assumptions`

Keyed by `id`. The relation is computed on `expression` exactly as for claims — the same order, on the same normal form, including the both-`iff`s-satisfied case, which classifies `unknown` for the same reason it does on `properties` (correction 17). Direction and policy valence differ: an assumption `strengthened` admits fewer environments and makes verification easier, and is the gaming move docs/50 names beside its fairness sibling ("adding a fairness assumption that schedules away the bug", plan §5.1) — the "Completeness guarantee" gaming-move table carries a row for it (correction 18). A change to `classification` or to `fidelity_profile` with an unchanged expression MUST classify `incomparable`: RFC 0002's fidelity profiles are explicitly not ordered, so no direction may be inferred. A change to `classification` or `fidelity_profile` together with a moved expression in the same revision classifies `unknown`, not `incomparable`, for the same reason as `properties`' `kind`/`observer` compound case: the content axis's decision procedure is never consulted once the unit's declaredness has also moved (correction 19).
...
- `kind`: `weak → strong` on the same action classifies `strengthened`, because strong fairness implies weak fairness; `strong → weak` classifies `weakened`. (A `kind` change is equivalently expressible as one `removed` plus one `added` record; a classifier MUST choose one encoding and MUST NOT emit both.) This single-record encoding applies only to a *clean* swap — nothing differs between the before-only and after-only candidate units except `kind`, i.e. `condition` is structurally identical on both sides. A same-action swap whose `condition` also moved is not "equivalently expressible" as one constraint's kind changing: a `strengthened`/`weakened` record carries one direction, and the `kind` and `condition` axes may disagree in sign, so no single record can state both without inventing a net direction the classification lattice does not define. Such a pair falls outside the exception and back under the "one record per classified unit" rule above: it classifies as an ordinary `removed`+`added` pair, never a single `kind`-change record (correction 16).
...
- `no-downgrade` blocks exactly `downgraded` — `AssuranceChange::blocked_by_no_downgrade`. It does not block `upgraded`, and it does not by itself block membership changes to `accepted_evidence_classes`; those are governed by the same verb through the `review` path and by the fail-closed rule. Normative: an `assurance` record whose relation is `added` or `removed` contributes `review` under `no-downgrade` and under `review`, on either acceptance path. Under `proposal-only` it keeps P4's authority: `allow` only on a path where a human principal performs `intent.accept`, and `review` otherwise, because that human acceptance is the review the verb requires. It contributes `block` under `locked` and `allow` under `unlocked`. It never contributes `allow` under `no-downgrade` (correction 20).
...
The eight reward-hacking moves of plan §5.1 map onto the lattice as follows, and each maps to the verb that blocks it. A ninth row, sourced from docs/50 and the `assumptions` section above rather than from plan §5.1 (whose eight bullets name a fairness-assumption addition but no bare assumption edit), is appended for the assumption-strengthening G3 dimension this section's opening paragraph already names (correction 18):

| Gaming move (plan §5.1, plus docs/50 for the ninth row) | Field | Relation | Blocking verb (plan §5.4) |
|---|---|---|---|
...
| narrow an assumption's expression to admit fewer environments | `assumptions` | `strengthened` | `review` |
...
16. **The `kind` bullet's same-action collapse had no stated scope, so it read as licensing a collapse across a simultaneous `condition` edit too.** The field table gives `fairness`'s unit of classification as "one constraint, keyed by (`kind`, `action`)" — `kind` is part of the key (RFC 0037 W1 keys uniqueness on exactly the same pair), so `weak:A` and `strong:A` are two distinct units, and by the "One record per classified unit" rule above ("MUST NOT collapse two units with different relations into one record") a before-only `weak:A` paired with an after-only `strong:A` classifies as two ordinary records, `removed` and `added`, by default. The `kind` bullet carves out one explicit exception, calling the pair "equivalently expressible" as a single `strengthened`/`weakened` record, but never stated what that phrase excludes, so a swap coincident with a `condition` edit read as either inside or outside it. Normative: the exception holds only for a *clean* swap — nothing but `kind` differs between the two candidate units. A same-action swap whose `condition` also moved is not equivalently expressible as one constraint's kind changing: the `kind` and `condition` axes may disagree in sign, and a `strengthened`/`weakened` record carries one direction, not two, so no single record can state both without inventing a net direction the classification lattice does not define. Such a pair falls back under the general "MUST NOT collapse" rule and classifies as `removed`+`added`, never a single `kind`-change record. Direction: the "`fairness`" section's `kind` bullet is extended to state the exception's scope. `crates/continuum-semantic-diff/src/fairness.rs` already implements this reading and no longer needs to carry its own disambiguating essay (bn-3vxp implemented the reading; bn-1fik9 corrected this RFC's text to state it).
17. **A proven equivalence under unequal canonical encodings satisfies both directional `iff`s.** The `properties` rule states a direction "`strengthened` iff behaviors(after) ⊆ behaviors(before), `weakened` iff behaviors(before) ⊆ behaviors(after)" (mirrored by `assumptions`' "the same order, on the same normal form"); CPNF-1 is sound but incomplete (RFC 0037 S1/S2), so two encodings can differ while both inclusions hold — a proven equivalence the normal form does not recognize as one encoding. Both `iff`s are then simultaneously satisfied, the text names no way to choose between them, the lattice has no `equivalent` token for the case, and R2 forbids claiming `unchanged` on anything short of encoding equality. Normative: such a pair classifies `unknown`. This is the safe polarity for a diff engine whose job is privileged-change detection: a proven equivalence is never itself the dangerous direction in either field's order (an equivalent property is neither more nor less restrictive than before; an equivalent assumption admits exactly the same environments), so failing closed to `unknown` never under-reports a weakening or a strengthening — it costs a review on a pair that provably needed none, the same cost S2 already prices in for every undecided pair. Minting a dedicated `equivalent` token is a candidate improvement, but it is wire-visible (the relation set is closed per "Versioning and revision" and extending it needs a major `schema_version` bump and a coordinated protocol change), so it is raised as F6 for the next protocol-minor bundle rather than decided here. Direction: the "`properties`" section is corrected to state the resolution explicitly, and the "`assumptions`" section is corrected to point at it. `crates/continuum-semantic-diff/src/properties.rs`'s `finite_formula_relation` already implements and tests this reading (bn-8mlg flagged the collision; bn-181aj corrects this RFC's text to state it).
18. **The "Completeness guarantee" gaming-move table carried no row for assumption strengthening.** The section's opening paragraph names "assumption strengthening" as one of the seven G3 dimensions, and docs/50 lists "strengthen assumptions" as its own attack class beside "strengthen fairness," but the table that followed mapped only plan §5.1's eight bullets, none of which is phrased as a bare assumption edit — its fairness bullet ("adding a fairness assumption that schedules away the bug") names `fairness`, not `assumptions`. Normative: the table gains a ninth row, sourced from the `assumptions` section's own rule and docs/50 rather than from plan §5.1: narrowing an assumption's expression to admit fewer environments is `assumptions`/`strengthened`, blocked by `review` under `assumptions`' base verb row (RFC 0037 W6) exactly as the `fairness` row above it is (both fields carry no directional verb of their own — `continuum_intent::change_policy::PolicyField::verbs` gives both the same base set). Direction: the "Completeness guarantee" table and its intro sentence are corrected to add the row and its source. `crates/continuum-semantic-diff/src/assumptions.rs`'s module doc already names the table's omission an editorial gap and states the relation and the field in prose (bn-7vg7 flagged the gap; bn-181aj corrects this RFC's text to close it).
19. **Neither `properties`' meaning axis nor `assumptions`' declaredness axis stated a rule for moving together with the content axis.** `properties` states `kind`/`observer` moving `incomparable` only "with an unchanged expression"; `assumptions` states `classification`/`fidelity_profile` moving `incomparable` only "with an unchanged expression" too; neither text said what a unit classifies when the named axis and the expression move in the same revision. Normative: such a compound move classifies `unknown`, not `incomparable` — the content axis's decision procedure (CPNF-1 inclusion, oracle-backed where a fragment license is held) answers a question about one claim's or one assumption's meaning under one fixed `kind`/`observer`/`classification`/`fidelity_profile`; once that axis has also moved, the two sides no longer share the meaning the procedure would be deciding a direction over, so the procedure is never consulted across the boundary and the record cannot affirm a direction it did not decide. This is the same safe polarity as correction 17: `unknown` is non-affirmative and fails closed to `review`/`block` under P2 exactly as `incomparable` does, so ratifying it here costs nothing the fail-closed rule does not already spend, and it never lets a compound edit under-report a privileged change by finding a spuriously affirmative direction. This resolution is distinct from `fairness`'s `kind`+`condition` compound (correction 16): there, `kind` is part of the unit's own key, so a `kind`+`condition` move is two units under that field's own keying and resolves to `removed`+`added`; here, `kind`/`observer`/`classification`/`fidelity_profile` are not part of either unit's key, so the unit is one unit throughout and the compound move resolves within a single record, never a pair. Direction: the "`properties`" and "`assumptions`" sections are each corrected to state the compound case explicitly. `crates/continuum-semantic-diff/src/properties.rs` and `crates/continuum-semantic-diff/src/assumptions.rs` already implement and test this reading, and the reading is unaffected by either module's later wiring to a decision procedure for the single-axis content case, since a compound move never reaches that procedure under this rule (bn-8mlg and bn-7vg7 independently flagged the same silence; bn-181aj corrects this RFC's text to state one rule for both).
20. **`accepted_evidence_classes` membership changes go through the `review` path.** The "Assurance movement" bullet said `no-downgrade` "does not by itself block membership changes to `accepted_evidence_classes`; those are governed by the same verb through the `review` path and by the fail-closed rule", but RFC 0037 P4 gives `no-downgrade` ordinary authority, so an agent acceptance of an added evidence class contributed `allow`. Adding an accepted evidence class loosens what satisfies the intent (INV-001, INV-011), and the bn-bpz0 C020 campaign found eight such mutants allowed under `no-downgrade`. Normative: an `assurance` record whose relation is `added` or `removed` contributes `review` under `no-downgrade` and under `review`, on either acceptance path. Under `proposal-only` it keeps P4's authority: `allow` only on a path where a human principal performs `intent.accept`, and `review` otherwise, because that human acceptance is the review the verb requires. It contributes `block` under `locked` and `allow` under `unlocked`. It never contributes `allow` under `no-downgrade`. The rule covers `removed` as well as `added`: the set is unordered and "MUST NOT be ranked" ("`assurance`"), so a verdict that allowed one membership direction and reviewed the other would rank it. On `assurance` only the membership half produces `added` or `removed`, so the field and the relation identify the record. `unlocked` keeps P4's `allow`, because its owner declared ordinary edits allowed. `proposal-only` keeps P4's path condition: a human principal's `intent.accept` is the review that verb requires, so forcing `review` over it adds no safety. Direction: this RFC refines RFC 0037 P4 for this one record shape, and RFC 0037 correction 18 records the pointer; `crates/continuum-intent/src/change_policy.rs` (`PolicyTable::verdict`) implements it (bn-36luu).

Flags raised against artifacts this RFC does not own (no silent divergence):

- **F1 — `intent_changes` records cannot name the classified unit.** The record shape is `{field, relation, protected, fragment, evidence}` with `additionalProperties: false`, so the wire cannot say *which* claim was weakened. The classifier computes per-unit relations, emits one record per non-`unchanged` unit, and carries the unit identity only in `policy.reasons` prose. A per-unit locator belongs in the schema; raised for the schema sweep (SD-08), not patched here. — **Paid by the bn-1lfek schema sweep, 2026-07-31**: `intent_changes[]` gained a `unit` string locator, required for the eleven `field` members whose unit-of-classification table declares a per-unit key; the four whole-field members (`bounds`, `trust_boundaries`, `completion_policy`, `assurance`) have no sub-unit to name and stay without one.
- **F2 — `requested_assurance` is an unconstrained string** in the normative schema. This RFC requires one of the five `AssuranceLevel` tokens; the schema does not yet enforce it. — **Paid by the bn-1lfek schema sweep, 2026-07-31**: `requested_assurance` is now the closed five-member `AssuranceLevel` enum, token-for-token `assurance.minimum` in `intent-contract.schema.json` and `AssuranceLevel` in `crates/continuum-value/src/assurance.rs`.
- **F3 — no classifier/CPNF version field.** The artifact records neither the classifier version nor the normal-form version, though both are part of the classification's identity under RFC 0030's query key. Raised for the schema sweep. — **Paid by the bn-1lfek schema sweep, 2026-07-31**: the schema gained an optional `classifier` block (`classifier_version` and `cpnf_version` required within it).
- **F4 — `policy.reasons` is free-form strings.** Typed per-field reasons would satisfy INV-003 structurally rather than by convention. — **Deferred.** The schema-side half is **paid by the bn-1lfek schema sweep, 2026-07-31**: the unit identity these strings used to carry as prose now has a typed home in `intent_changes[].unit`, so a reason string is no longer the only place a machine-consumable fact appears. The rest of F4 stays open: typing the reasons themselves needs a closed per-field reason vocabulary, and this RFC does not yet mint one — inventing one in the schema would make the schema the source of a vocabulary this RFC does not define. Minting that vocabulary is this RFC's to do, not a schema sweep's.
...
- **F6 — No `equivalent` token for a proven equivalence under unequal canonical encodings.** Correction 17 records that `properties`' (and, by the same order, `assumptions`') two directional `iff`s can both hold for one pair — CPNF-1 is sound but incomplete (S1/S2), so a bounded decision procedure can prove inclusion in both directions between two encodings that differ. The interim, correction-17 answer is `unknown`, which fails closed to `review`/`block` exactly as every other non-affirmative relation does and never under-reports a privileged change (a proven equivalence is never the dangerous direction in either field's order, so failing closed here costs a review, not a missed weakening or strengthening). Whether the closed relation set should instead grow a dedicated `equivalent` token — a member between `unchanged` and the directional pair, affirmable only on a discharged double inclusion — is a wire-vocabulary question, not a text question: the relation set is closed ("Versioning and revision"), and adding a member is a breaking change requiring a major `schema_version` bump and a coordinated revision of this RFC, RFC 0026's IDL consumers, and the normative schema together. Raised for the next protocol-minor bundle (bn-3jrtz) rather than decided here; `crates/continuum-semantic-diff/src/properties.rs`'s `finite_formula_relation` already implements and tests the interim `unknown` answer correction 17 ratifies, so resolving F6 later is additive — it narrows an existing `unknown` arm to a new token, not a change to what any current record means. — **Swept at that bundle (protocol 3.4, bn-3jrtz) and DEFERRED, with correction 17's blessing kept in force.** The bundle judged it not payable at a minor, on this entry's own terms: the relation set is closed, so minting `equivalent` is the *breaking* half of the F15/F18 pattern — a major `schema_version` bump of `semantic-diff.schema.json` plus a coordinated move of this RFC's classifier sections, `crates/continuum-semantic-diff`'s per-field classifiers, the `intent_changes[]` assembler, and P2's policy valence for the new member, none of which a protocol-minor bundle's mechanism (compatible additions riding one bump) can carry. Nothing is held hostage by waiting: correction 17's `unknown` is the safe polarity — it never under-reports a privileged change and costs exactly one review on a pair that provably needed none — and this entry's own closing sentence stays true, so a future major that takes the token narrows an existing `unknown` arm without reinterpreting any published record.
