...
A12's concern names a real asymmetry worth recording even though it is not a violation. `task.resume`'s own terminal short-circuit (`daemon::task::resume`, ratified for A1/A2 by bn-10093) answers the identical situation — an operation reached against an already-terminal task — differently: it reports `status = ok` on the task-observing lane (`reported(...).observing(...)`), naming the terminal task without ever touching the `task_started`/`task_suspended` lane. `verification.start` has no equivalent branch: its cached-result lane checks only `TaskStatus::Completed`, so `Failed` and `Cancelled` both fall through to the same `task_started` answer a freshly created task gets. The two operations answer one fact — "this call's answer names a task that is already terminal" — two different ways. Mirroring `task.resume`'s shape in `verification.start` would close A12's concern completely, but it is not an additive change: it would flip an already-reachable scenario's `ResultStatus` from `task_started` to `ok`, which is a wire-visible behavior change, not a new field. Recorded as **F20** in the flag ledger, for the next protocol-minor bundle rather than proposed here.
