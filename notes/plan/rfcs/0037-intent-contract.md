...
- **P4.** Otherwise the verb's acceptance authority applies: `unlocked` contributes `allow`; `proposal-only` contributes `allow` only on a path where a human principal performs `intent.accept`, and `review` otherwise; `review` contributes `review` for any non-`unchanged` relation and names the reviewers from `policy_reviewers`; `locked` contributes `block` for any non-`unchanged` relation. RFC 0031 correction 20 refines this step for `assurance` records whose relation is `added` or `removed`: they contribute `review` under `no-downgrade` on either acceptance path, and `proposal-only` keeps the path condition above (correction 18).
...
18. **P4 gave `no-downgrade` ordinary authority over `accepted_evidence_classes` membership changes.** P4 made `no-downgrade` contribute `allow` for every relation outside its denied set, so an agent acceptance that added an accepted evidence class was allowed, though RFC 0031 routes membership changes through the `review` path. Normative: RFC 0031 correction 20 governs. An `assurance` record whose relation is `added` or `removed` contributes `review` under `no-downgrade` and under `review`, on either acceptance path. Under `proposal-only` it keeps this step's authority: `allow` only on a path where a human principal performs `intent.accept`, and `review` otherwise. It contributes `block` under `locked` and `allow` under `unlocked`. Direction: P4 carries a pointer to that correction; `PolicyTable::verdict` implements it (bn-36luu).
...
- **F3 — `policy_reviewers` is unconstrained.** Its keys are `additionalProperties` rather than the fifteen policy keys, and nothing ties a `review` verb to a non-empty principal list (W7). — The schema-side half is **paid by the bn-1lfek schema sweep, 2026-07-31**: `policy_reviewers`'s keys are now restricted to the fifteen policy keys via `propertyNames`, and a named-but-empty principal list is rejected via `minItems: 1`. The other half of W7 stays open: that every field whose verb is `review` carries a non-empty list is a relation between the keys of one object and the values of another, which JSON Schema cannot state directly and which remains a checker rule.
- **F4 — Any verb validates on any field.** `policy_verb` is one enum applied to all fifteen keys, so the applicability matrix (W6) is prose today; per-field verb enums would make it structural. — **Paid by the bn-1lfek schema sweep, 2026-07-31**: the `policy` object's fifteen keys now carry per-field verb restrictions (`policy_verb_base`/`policy_verb_bounds`/`policy_verb_trust_boundaries`/`policy_verb_assurance`/`policy_verb_removal`) encoding the closed W6 applicability matrix, so an inapplicable verb — `no-decrease` on `properties`, or `no-downgrade` outside `assurance` — is unrepresentable rather than merely rejected by prose.
- **F5 — `scope.fragments` has no `minItems`.** An empty declaration would make every unit `unsupported` and every promotion blocked — fail-closed, but silently (W3). — **Paid by the bn-1lfek schema sweep, 2026-07-31**: `scope.fragments` now carries `minItems: 1`.
...
- **F10 — The registry record carries no lineage.** `intent-registry-record.schema.json` has no predecessor or `superseded_by` field, so the common ancestor the three-way merge requires (M1) is not recoverable from the record alone. — **Paid by the bn-1lfek schema sweep, 2026-07-31**: the schema gained optional nullable `supersedes` and `superseded_by` fields (`^in_`), and a record whose `status` is `superseded` is now required to name its successor in `superseded_by`.
- **F11 — Prose case in the contract schema.** The `scope.fragments` description writes "classification is Unknown"; the wire literal is lowercase (correction 1). No shape impact; raised for the schema sweep. — **Paid by the bn-1lfek schema sweep, 2026-07-31**: the `scope.fragments` description was corrected to the lowercase wire literal, `unknown`.
- **F12 — No landed lane discharges the non-vacuity obligation.** AO2 names the caller with standing — the lane that assembles a synthesis task — and `continuum-forge` is still a PR-1/IMPL-01 scaffold, so nothing outside tests calls `Optimization::require_non_vacuity` today. The obligation is implemented and unreferenced until the Forge lane lands. RFC 0033 owns the surface that must call it and the typed refusal that surface returns; this RFC fixes only that the requirement exists and where it does not belong. — **Discharged by the bn-1dsih Forge task-assembly lane, 2026-08-09**: `crates/continuum-forge/src/task.rs`'s `assemble` is AO2's caller with standing. It makes both calls on every invocation — `IntentContract::check` for the document's five cross-field rules and `Optimization::require_non_vacuity` for INV-012 — and refuses an empty set with a typed `TaskRefusal::Vacuous` whose `invariant` accessor returns the token `INV-012` as data rather than as message prose. Both calls run before either decides, so a contract that fails both reports both grounds and "the second call was made" is a property of the returned value. `crates/continuum-forge/tests/non_vacuity_second_call.rs` holds the first call's clean verdict and the lane's refusal side by side on the same bytes, which is correction 17's argument executed rather than restated. RFC 0033 still owns the wire spelling and none was invented here: the IDL declares no error code for a refused synthesis task and no `forge` namespace, so the refusal stays a crate-level typed value until that verb lands.
