...
//! | `config::RunConfig::parse` (not budgeted: it runs before a lowering exists) | the input is at most [`config::MAX_CONFIG_BYTES`]; a sort at most [`config::MAX_SORT_ELEMENTS`] elements | the strict JSON reader's depth bound (64); `read_value` recurses within it | linear in the bounded input, plus a sort of each `set`/`map` by canonical bytes (`n log n` comparisons; each level re-encodes its members, so at most depth × input bytes) |
...
//! | `lower`: model-core's limits (`preflight`) | none; refuses before init enumeration and before any action: variables > `MAX_VARIABLES`, total expanded actions > `MAX_ACTIONS`, a declared or generated name > `MAX_IDENT_BYTES`, one action's candidates > [`crate::lower::MAX_RELATIONAL_CANDIDATES`] | flat | one table lookup per action entry and one unit per name, charged as it runs |
