...
//! | `lower`: fairness assumptions (`lower_fairness`, `fairness_site`, bn-1ln12) | the action and choice tables (a reference per entry, charged before they are built); a choice's offered actions (a reference each, charged); per assumption one planned site: per scope member its name, the builder's resolved copy, and its scope index, charged in one step before the first name is written; the count of assumptions checked against `MAX_FAIRNESS` in `preflight`, before any scope | flat | the tables' sort and each lookup spent first; the plan's work (per instance its values' text, per member writing and resolving its name) checked against the work left before the first name, spent exactly; the builder's per-member binary search and its sorts of members and assumptions charged before `build` |
