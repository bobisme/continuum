...
//! `crates/continuum-kernel-temporal/tests/receipt.golden.json` is the byte-for-byte
//! output for the drain-ranking fixture; `tools/check_kernel_covenant.py`
...
//! share an ordinal: `CONTTMPC 1`, `CONTSATC 1`, `CONTSMTC 1`, `CONTTMPC 1`. A
...
//! [`WIRE_EPOCH_ID`], `continuum-kernel-temporal/CONTTMPC/1` — and it is carried in
...
//!
//! # `claim.kind` is derived, not supplied
//!
//! Both families this crate checks are progress statements: a ranking discharges
//! docs/16 PO-LIV-002/003 and a fair-SCC exclusion discharges PO-LIV-004. The claim
//! kind is therefore `liveness` by construction, and no caller can relabel it — which
//! is the difference from `continuum-kernel-sat` and `continuum-kernel-smt`, where a
//! refutation cannot see the obligation it discharges.
//!
...
pub const CHECKER_NAME: &str = "continuum-kernel-temporal";
...
pub const WIRE_EPOCH_ID: &str = "continuum-kernel-temporal/CONTTMPC/1";
...
impl Receipt {
    ...
    pub fn to_json(&self) -> String {
        ...
        out.push_str("  \"claim\": {\n");
        field(&mut out, 2, "kind", "liveness", true);
...
const fn schema_certificate_kind(kind: CertificateKind) -> &'static str {
    match kind {
        CertificateKind::Ranking => "ranking",
        CertificateKind::FairSccExclusion => "fair-scc-exclusion",
    }
}
...
fn claim_text(claim: &CheckedClaim) -> String {
    ...
    match claim.kind() {
        CertificateKind::Ranking => {
            out.push_str(
                "ranking certificate: every non-goal state has a successor of strictly \
                 smaller rank and every goal state ranks 0, so progress is \
                 unconditional and no fairness assumption is discharged",
            );
        }
        CertificateKind::FairSccExclusion => {
            out.push_str(
                "fair-scc-exclusion certificate: reachability re-derived by BFS and the \
                 strongly connected components of the reachable non-goal subgraph \
                 re-derived by Tarjan, with no nontrivial component fair under the \
                 declared fairness",
            );
        }
    }
    out.push_str(&format!(
        "; {} states ({} initial, {} goal, {} reachable) and {} transitions; property \
         class {}; fairness {} over {} declared fair actions",
        claim.states(),
...
mod tests {
    ...
    const SEAM: Seam<'static> = Seam {
        receipt_id: "receipt_drain-ranking-0001",
        certificate_digest: "blake3:cert-drain-ranking",
        observers_digest: "blake3:drain-observers",
        proof_epoch: "proof-2026-07",
        source_digest: "blake3:kernel-temporal-source",
        binary_digest: "blake3:kernel-temporal-binary",
        toolchain: "rustc 1.97.0 (pinned by rust-toolchain.toml)",
        reproduce: &["continuum proof check --receipt receipt_drain-ranking-0001"],
    };
    ...
    #[test]
    fn a_verified_ranking_certificate_yields_the_checked_in_golden_receipt() {
        let verdict = verified(&Plan::drain_ranking());
    ...
    fn the_receipt_names_which_crates_wire_epoch_it_means() {
        ...
        assert_eq!(
            built.wire_epoch_id(),
            "continuum-kernel-temporal/CONTTMPC/1"
        );
        assert_eq!(built.checker_name(), "continuum-kernel-temporal");
        assert!(built.checker_version().ends_with("+wire.CONTTMPC.1"));
        ...
        assert!(!epochs.contains("CONTTMPC"));
    ...
    fn the_envelope_supplies_the_hashes_the_certificate_carries() {
        ...
        assert!(json.contains("\"model_hash\": \"blake3:drain-model\""));
        assert!(json.contains("\"property_hash\": \"blake3:eventually-goal\""));
        // …and the seam supplies the ones no certificate carries.
        assert!(json.contains("\"observers_hash\": \"blake3:drain-observers\""));
        assert!(json.contains("\"binary_hash\": \"blake3:kernel-temporal-binary\""));
    }

    #[test]
    fn the_two_families_report_their_own_schema_kind_and_their_own_claim() {
        let ranking = receipt(&verified(&Plan::drain_ranking()), &SEAM).unwrap();
        assert_eq!(ranking.certificate_kind(), "ranking");
        assert!(
            ranking
                .claim_text()
                .contains("no fairness assumption is discharged")
        );

        let scc = receipt(&verified(&Plan::fair_progress_exclusion()), &SEAM).unwrap();
        assert_eq!(scc.certificate_kind(), "fair-scc-exclusion");
    ...
    #[test]
    fn the_claim_kind_is_liveness_and_no_caller_can_relabel_it() {
        let json = receipt(&verified(&Plan::drain_ranking()), &SEAM)
            .unwrap()
            .to_json();
        assert!(json.contains("\"kind\": \"liveness\""));
    }
    ...
    fn an_unsupported_certificate_has_no_receipt() {
        // A ranking that declares fair actions asks for the fairness-to-progress
        // linkage this build does not implement: valid, perhaps, but not checked here.
        let mut plan = Plan::drain_ranking();
    ...
    fn a_receipt_id_outside_the_schema_pattern_is_refused() {
        let verdict = verified(&Plan::drain_ranking());
        for bad in ["drain-0001", "receipt_", "receipt_bad id", "receipt_bad.id"] {
            let mut seam = SEAM;
            seam.receipt_id = bad;
            assert!(
