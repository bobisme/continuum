...
mod no_widening {
    ...
    ///
    /// bn-18w74 re-derived it for the keystore's custody surface: the state file codec
    /// (`encode_state`, `decode_state`), `persist` and `sweep`, and the store's
    /// `SigningCustody` implementation. The daemon reaches them only through the
