...
//!
//! `protocol.version` is `"3.9"` as of IDL 1.17, and the whole claim of a *compatible*
...
//!
//! 3.4 (bn-3jrtz) pays F19 and F20 and moves nothing here either: a new `@since("3.4")`
//! struct behind an `optional` field is the ordinary compatible addition, and F20's lane
//! flip moves no declaration. A client pinned to 3.3 — or 3.0 — is still served exactly
//! that version, which is what the pinned-minor test below holds.
//!
//! 3.5 (`whiteboard.compile`, bn-1as8e) is the second minor that adds an operation, and
...
//!
//! 3.7 (`CapabilityDescriptor.instances`, bn-28kv4) adds an `optional` field and no
//! operation. `implemented()` gains 3.7, a client pinned to 3.6 is still served 3.6, and
//! the field is kept off such a connection's `ServerWelcome.grant`
//! (`gate_g1_07_acceptance.rs`, `instance_scope`).
//!
//! 3.8 (the signing wire, bn-3glnv) adds eight operations and one `optional` field.
//! `implemented()` gains 3.8, a client pinned to 3.7 is still served 3.7, the eight
//! operations are refused on such a connection, and `EvidenceGetResponse.signature` is
//! kept off it (`daemon_signing.rs`).
...
///
/// `3.0` through `3.8` are all here for the same reason one level up: a daemon at the new
...
fn implemented() -> Vec<ProtocolVersion> {
    vec![
        ...
        ProtocolVersion::new(3, 7),
        ProtocolVersion::new(3, 8),
        ProtocolVersion::new(3, 9),
    ]
}
...
fn the_declared_version_is_the_idls() {
    ...
    // bump: the constant reads 3.9 (bn-18w74, the `OutcomeUnknown` code), and 3.9 is a
    // version the daemon of this test can actually negotiate.
...
fn a_client_pinned_to_the_previous_minor_still_gets_it() {
    ...
    // And the 3.7 obligation, one minor on: exactly 3.6 is served as 3.6.
    let pinned = open((3, 6), (3, 6)).expect("3.6 is still implemented and served");
    assert_eq!(pinned.protocol_version(), ProtocolVersion::new(3, 6));
    assert!(
        !pinned.admits_request(ProtocolVersion::new(3, 7)),
        "a connection negotiated at 3.6 does not admit a 3.7 request"
    );

    // And the 3.8 obligation, one minor on: exactly 3.7 is served as 3.7.
    let pinned = open((3, 7), (3, 7)).expect("3.7 is still implemented and served");
    assert_eq!(pinned.protocol_version(), ProtocolVersion::new(3, 7));
    assert!(
        !pinned.admits_request(ProtocolVersion::new(3, 8)),
        "a connection negotiated at 3.7 does not admit a 3.8 request"
    );

    // And the 3.9 obligation, one minor on: exactly 3.8 is served as 3.8, so a 3.8 client
    // is never sent `OutcomeUnknown` (`rule signing.custody`).
    let pinned = open((3, 8), (3, 8)).expect("3.8 is still implemented and served");
    assert_eq!(pinned.protocol_version(), ProtocolVersion::new(3, 8));
    assert!(
        !pinned.admits_request(ProtocolVersion::new(3, 9)),
        "a connection negotiated at 3.8 does not admit a 3.9 request"
    );
}
...
fn a_range_spanning_the_window_selects_the_highest_served_version() {
    ...
    let negotiated = open((1, 0), (9, 9)).expect("3.9 is common and served");
    assert_eq!(negotiated.protocol_version(), ProtocolVersion::new(3, 9));
}
