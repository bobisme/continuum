...
///
/// `3.0` and `3.1` are both here for the same reason one level up: a daemon at the new
/// minor still implements the old one, and a client pinned to `3.0` must still be served
...
fn the_declared_version_is_the_idls() {
    ...
    // bump: the constant reads 3.1, and 3.1 is a version the daemon of this test can
    // actually negotiate.
    assert_eq!(PROTOCOL_VERSION, "3.1");
...
fn a_range_spanning_the_window_selects_the_highest_served_version() {
    ...
    let negotiated = open((1, 0), (9, 9)).expect("3.1 is common and served");
    assert_eq!(negotiated.protocol_version(), ProtocolVersion::new(3, 1));
}
